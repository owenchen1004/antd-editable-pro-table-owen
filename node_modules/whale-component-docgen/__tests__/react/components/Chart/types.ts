type View = 'View';
type ViewPadding = 'ViewPadding';
type ScaleOption = 'ScaleOption';
type Datum = 'Datum';
type SizeAttrCallback = 'SizeAttrCallback';
type AdjustOption = 'AdjustOption';
type ColorAttrCallback = 'ColorAttrCallback';
type ShapeAttrCallback = 'ShapeAttrCallback';
type StyleOption = 'StyleOption';
type LooseObject = 'LooseObject';
type StyleCallback = 'StyleCallback';
type GeometryTooltipOption = 'GeometryTooltipOption';
type TooltipCallback = 'TooltipCallback';
type AnimateOption = 'AnimateOption';
type LabelOption = 'LabelOption';
type GeometryLabelCfg = 'GeometryLabelCfg';
type LabelCallback = 'LabelCallback';
type StateOption = 'StateOption';

interface IEvent {
  /** 触发的对象，图形或者 Canvas 对象 */
  target?: any;
  /** 当前 target 所属的 view */
  view?: View;
  /** 从底层 G 的绘图层抛出来的事件，详情参考：G 的事件详解 */
  gEvent?: any;
  /** dom 层的事件 */
  event?: any;
  /** 触发的位置 x，相对于画布左上角的位置 */
 ...
 */
interface IViewProps extends React.ComponentPropsWithRef<any> {
  /** 数据源配置。 */
  data?: any[];
   ...
   */
  filter?:
    | Array<[string, null | ((value: any, datum: Datum) => boolean)]>
    | Record<string, (value: any, datum: Datum) => boolean>;
}
...
interface IBaseGemoProps {
  ...
  shape?:
    | ShapeString
    | FieldString
    | [ShapeString, ShapeString[] | ShapeAttrCallback];
  /**
   *
   * @example
   * ```typescript
  ...
  label?:
    ...
    | false
    | FieldString
    | [FieldString, GeometryLabelCfg | LabelCallback]
    | [FieldString, LabelCallback, GeometryLabelCfg];
  /**
   *设置状态对应的样式。
   *
   * @example
