    ...
    if (value[0] === '-' && value.lastIndexOf('-') > 0 || value.includes('safe')) {
      ...
      if (value.includes('url')) {
        ...
        value = value.replace(/[0-9.]+\s*rpx/g, function ($) {
          return parseFloat($) * utils_1.windowWidth / 750 + "px";
        });
      }
...
HtmlParser.prototype.onOpenTag = function (selfClose) {
  ...
  if (close) {
    ...
    if (node.name === 'img') {
        ...
        if (!attrs.ignore || node.webp || attrs.src.includes('cloud://')) {
          for (var i = this.stack.length; i--;) {
            ...
            if (style.includes('flex:') && !style.includes('flex:0') && !style.includes('flex: 0') && (!styleObj.width || !styleObj.width.includes('%'))) {
              ...
              for (var j = i + 1; j < this.stack.length; j++) {
                var _style = this.stack[j].attrs.style || '';
      ...
      if (parseInt(styleObj.width) > utils_1.windowWidth) {
...
HtmlParser.prototype.popNode = function () {
  ...
  if (node.name === 'svg') {
    ...
    node.name = 'img';
    node.attrs = {
      src: "data:image/svg+xml;utf8," + src_1.replace(/#/g, '%23'),
      style: style,
      ignore: 'T'
    };
  ...
  if (node.name !== 'table' && parseInt(styleObj.width) > utils_1.windowWidth) {
  ...
  if (config.blockTags[node.name]) {
    ...
    var padding_1 = parseFloat(attrs.cellpadding);
    var spacing = parseFloat(attrs.cellspacing);
    var border_1 = parseFloat(attrs.border);
    node.c = 1;
    ...
    if (border_1) {
      attrs.style += ";border:" + border_1 + "px solid gray";
    }
    ...
    if (node.flag && node.c) {
      ...
      for (var row = 1; row <= trList_1.length; row++) {
        var col = 1;

        for (var j = 0; j < trList_1[row - 1].children.length; j++, col++) {
          var td = trList_1[row - 1].children[j];
          ...
          if (td.name === 'td' || td.name === 'th') {
            ...
            td.c = 1;
            var style = td.attrs.style || '';
            var start = style.indexOf('width') ? style.indexOf(';width') : 0;

            if (start !== -1) {
              var end = style.indexOf(';', start + 6);
            ...
            style += (border_1 ? ";border:" + border_1 + "px solid gray" + (spacing ? '' : ';border-right:0;border-bottom:0') : '') + (padding_1 ? ";padding:" + padding_1 + "px" : '');
        ...
        (function traversal(nodes) {
          for (var i = 0; i < nodes.length; i++) {
            ...
            if (td.name === 'th' || td.name === 'td') {
              if (border_1) {
                td.attrs.style = "border:" + border_1 + "px solid gray;" + (td.attrs.style || '');
              }

              if (padding_1) {
                td.attrs.style = "padding:" + padding_1 + "px;" + (td.attrs.style || '');
              }
            } else if (td.children) {
...
HtmlParser.prototype.onText = function (text) {
  ...
  if (this.hook(node)) {
    if (this.options.selectable === 'force' && utils_1.isIOS) {
      this.expose();
      node.us = 'T';
    }
