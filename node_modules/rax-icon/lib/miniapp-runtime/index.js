"use strict";

exports.__esModule = true;
exports.createIconSet = createIconSet;
exports.default = void 0;

var _rax = require("rax");

var _universalEnv = require("universal-env");

var _raxText = _interopRequireDefault(require("rax-text"));

var _raxImage = _interopRequireDefault(require("rax-image"));

var _wrapDefaultProperties = _interopRequireDefault(require("../utils/wrapDefaultProperties"));

var _createIconSet = require("../createIconSet");

var _excluded = ["source", "fontFamily", "style"];

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _extends() { _extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; }; return _extends.apply(this, arguments); }

function _objectWithoutPropertiesLoose(source, excluded) { if (source == null) return {}; var target = {}; var sourceKeys = Object.keys(source); var key, i; for (i = 0; i < sourceKeys.length; i++) { key = sourceKeys[i]; if (excluded.indexOf(key) >= 0) continue; target[key] = source[key]; } return target; }

var fontCache = new Map();
var Icon = (0, _rax.forwardRef)(function (_ref, ref) {
  var _ref$source = _ref.source;
  _ref$source = _ref$source === void 0 ? {} : _ref$source;

  var uri = _ref$source.uri,
      codePoint = _ref$source.codePoint,
      fontFamily = _ref.fontFamily,
      _ref$style = _ref.style,
      style = _ref$style === void 0 ? {} : _ref$style,
      rest = _objectWithoutPropertiesLoose(_ref, _excluded);

  if (uri && !codePoint && !fontFamily) {
    return (0, _rax.createElement)(_raxImage.default, _extends({}, rest, {
      source: {
        uri: uri
      },
      style: style
    }));
  }

  if (!fontFamily) {
    return (0, _rax.createElement)(_raxText.default, _extends({}, rest, {
      ref: ref,
      style: style
    }), codePoint);
  }

  var fontFile = fontCache.get(fontFamily);

  if (!fontFile) {
    // In Miniapp, iconfont must be loaded every time when page changes
    var source = "url('" + uri + "')";

    if (_universalEnv.isMiniApp) {
      if (typeof my.loadFontFace === 'function') {
        my.loadFontFace({
          family: fontFamily,
          source: source
        });
      } else {
        console.warn('Your container may not support my.loadFontFace! Please check it and use local fontfamily.');
      }
    } else if (_universalEnv.isWeChatMiniProgram) {
      wx.loadFontFace({
        family: fontFamily,
        source: source
      });
    } else if (_universalEnv.isByteDanceMicroApp) {
      tt.loadFont(uri);
    } else if (_universalEnv.isBaiduSmartProgram || _universalEnv.isKuaiShouMiniProgram) {
      console.warn('Current container does not support loading font face from uri.');
    }
  } else if (fontFile !== uri) {
    console.error("font-family " + fontFamily + " should be unique!");
    return null;
  }

  return (0, _rax.createElement)(_raxText.default, _extends({}, rest, {
    ref: ref,
    style: _extends({}, style, {
      fontFamily: fontFamily
    })
  }), codePoint);
});

function createIconSet(glyphMap, fontFamily, fontFile) {
  if (glyphMap === void 0) {
    glyphMap = {};
  }

  return (0, _createIconSet.createIconSet)(Icon, glyphMap, fontFamily, fontFile);
}

var _default = (0, _wrapDefaultProperties.default)(Icon);

exports.default = _default;