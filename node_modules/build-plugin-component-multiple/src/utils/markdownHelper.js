...
exports.markdownParser = function markdownParser(md, options = {}) {
  const { sliceCode, sliceApi, demoPath, snippets = {} } = options;
  function split(str) {
    ...
    if (jsxMatched) {
      ...
      if (demoMathed && demoMathed[1] && demoPath) {
        try {
          ...
          console.log(`[error] 获取 Demo 代码失败 ${demoMathed[1]}`);
        }
      }
  ...
  if (sliceApi) {
    const API_REG = /<API(\s)*\/>/;
    const apiMatched = API_REG.exec(result.content);

    if (apiMatched) {
      result.content = result.content.replace(API_REG, snippets.api || '\n\n');
    } else {
      // 自动在尾部附加 API
