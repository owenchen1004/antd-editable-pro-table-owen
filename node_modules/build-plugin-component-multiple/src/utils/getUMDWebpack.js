...
module.exports = ({ context, compileOptions }) => {
  ...
  const jsPath = path.resolve(rootDir, `src/index.tsx`);
  const cssPath = path.resolve(rootDir, `src/index.scss`);
  ...
  if (externals) {
    ...
    const validBasicComponent = (basicComponents ? [...basicComponents, ...defaultDynamicImportLibraries] : [])
      ...
      .filter(
        (externalKey) => Object.keys(externals).includes(externalKey) && typeof externals[externalKey] !== 'function',
      );
  ...
  if (sourceMap) {
    ...
    config.optimization.minimizer('TerserPlugin').tap(([options]) => [{ ...options, sourceMap: true }]);
  }
  // minify
  if (!minify) {
    // disable minify code
