...
const parseStyleStatement = ({ target, module, rootDir }) => {
  ...
  if (styleStatement) {
    return target === 'es' ? `import '${styleStatement}';` : `require('${styleStatement}');`;
  }
  return '';
};
...
module.exports = ({ rootDir, basicComponents, destPath, target, log, folder }) => {
  ...
  ['index.scss', 'index.less', 'index.css', 'main.scss'].every((cssFile) => {
    if (fs.existsSync(path.join(destPath, cssFile))) {
      styleContent = target === 'es' ? `import './${cssFile}';` : `require('./${cssFile}');`;
      // return false to break loop
      return false;
    }
    return true;
  });
  const styleSatements = styleDependencies.map((module) => parseStyleStatement({ module, rootDir, target })).join('\n');
  styleContent =
    styleSatements || styleContent ? [styleSatements, styleContent].filter(Boolean).join('\n') : '//empty file';
