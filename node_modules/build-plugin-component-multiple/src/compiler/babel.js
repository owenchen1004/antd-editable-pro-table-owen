...
const getBabelConfig = ({ target, componentLibs, rootDir, babelPlugins, babelOptions, type, alias }) => {
  ...
  if (type === 'react') {
    ...
    babelConfig.presets.unshift([require.resolve('@babel/preset-typescript'), { jsxPragma: 'createElement' }]);
    babelConfig = babelMerge(babelConfig, {
  ...
  componentLibs.forEach((libraryName) => {
    ...
    plugins.push([require.resolve('babel-plugin-import'), pluginOption, libraryName]);
  });
...
module.exports = function babelCompiler(context, log, basicComponents, userOptions = {}, type) {
  ...
  if (declaration) {
    const dtsCompileInfo =
      type === 'rax'
        ? // rax 模式下应业务方需求，需要为 es、lib 目录下加上 dts 文件
          [
            ...compileInfo.map((item) => ({
              ...item,
              destPath: path.resolve(item.destPath, '../es'),
            })),
            ...compileInfo.map((item) => ({
              ...item,
              destPath: path.resolve(item.destPath, '../lib'),
            })),
            ...compileInfo.map((item) => ({
              ...item,
              destPath: path.resolve(item.destPath, '../types'),
            })),
          ]
        : compileInfo.map((item) => ({
            ...item,
    ...
    dtsCompiler(dtsCompileInfo, log);
  }
