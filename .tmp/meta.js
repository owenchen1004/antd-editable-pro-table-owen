import EditProTableMeta from 'D:\\lowcode platform\\material\\zw-edit-protable\\zw-edit-protable\\lowcode\\edit-pro-table\\meta'
...
function fillRealVersion(meta, packageName = 'zw-edit-protable', version = '1.0.6', basicLibraryVersion={"@alifd/next":"1.25.23","@alifd/meet":"2.6.3","antd":"4.17.3"}) {
  if (!meta || !version) {
    return meta;
  }
  const { npm } = meta;
...
const componentMetas = [EditProTableMeta];
const components = [];
const npmInfo = {};
...
if (!execCompile) {
  window.ZwEditProtableMeta = { components, componentList };
}